...
\author%
{%
	\sc{P. Farrell} *, \sc{S. Zampini}$\;\dagger$, \underline{\sc{U. Zerbinati}} *\\
}
\institute%
{%
...
\date[Firedrake 2023]{Firedrake User Meeting, Exeter, 4th -- 6th of January 2023} % short date for footer
...
\begin{document}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item[\color{oxfordblue}$\blacktriangleright$] Meshing using an advancing front method,
			\item[\color{purple}$\blacktriangleright$] Discretising a Partial Differential Equation (\textbf{PDE}),
			\item[\color{purple}$\blacktriangleright$] Solving the linear or nonlinear system arsing from the discretisation.
		\end{itemize}
	...
	\begin{frame}
		\frametitle{Getting Started -- Installing NetGen}
			...
			\begin{alertblock}{NetGen}
				If installing from scratch use the NetGen repository from the Firdrake project GitHub.
			\end{alertblock}
	\end{frame}
	\begin{frame}
	...
	\begin{frame}
		...
		\begin{minipage}{0.25\textwidth}
			...
			\begin{figure}
				\centering
				\includegraphics[scale=0.2]{Figures/OCCBottleBody}
			\end{figure}
		\end{minipage}
	\end{frame}
	\begin{frame}
		\frametitle{The OCCT Kernel -- The Flask Example: III}
		$\newline$
		\lstinputlisting[language=Python,firstline=29,lastline=43]{Codes/OCCBottle.py}
	\end{frame}
	\begin{frame}
		\frametitle{The OCCT Kernel -- The Flask Example: III}
		\begin{figure}
			\centering
			\includegraphics[scale=0.24]{Figures/OCCBottle}
			\qquad
			\qquad
			\includegraphics[scale=0.28]{Figures/OCCStrein}
		\end{figure}
	\end{frame}
	\begin{frame}
